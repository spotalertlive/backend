...
import { dbGet, dbRun } from "../dbAsync.js";
  ...
  try {
    ...
    await dbRun(
      `INSERT INTO users
       ...
       VALUES (?, ?, ?, ?, ?, ?)`,
      [name, email, hash, "Free Trial", 0, verifyToken]
    );
    ...
    await getTransporter().sendMail({
      ...
      html: `<p>Welcome to SpotAlert</p>
<p>Verify your email:</p>
<a href="${verifyUrl}">${verifyUrl}</a>`,
    });

    return res.json({ message: "Signup successful. Verify email." });
  } catch (err) {
...
router.get("/verify", async (req, res) => {
  try {
    ...
    const user = await dbGet("SELECT id FROM users WHERE verify_token = ?", [token]);
...
router.post("/request-reset", async (req, res) => {
  try {
    ...
    await dbRun("UPDATE users SET reset_token = ? WHERE id = ?", [token, user.id]);
...
/* =========================
   ALIAS: forgot-password -> request-reset
========================= */
router.post("/forgot-password", async (req, res) => {
  req.url = "/request-reset";
