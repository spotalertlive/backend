...
router.post("/signup", async (req, res) => {
  try {
    ...
    await db.run(
      `INSERT INTO users 
       (name,email,password_hash,plan,reset_token,email_verified)
       VALUES (?,?,?,?,?,?)`,
      name,
      email,
      hash,
...
https://spotalert.live/verify.html?token=${verifyToken}
...
router.post("/verify", async (req, res) => {
  try {
    ...
    await db.run(
      "UPDATE users SET reset_token=NULL, email_verified=1 WHERE id=?",
      user.id
    );
