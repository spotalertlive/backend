// server.js — SpotAlert FULL backend (SQLite + AWS + Auth + Zones + Cost SAFE)
...
// ---------- AWS clients (SAFE: may still work if IAM role exists) ----------
...
async function initDb() {
  ...
  await db.exec(`
    ...
    -- Known faces tables (required by routes/known-faces.js)
    ...
    -- ✅ NEW: LOCATIONS (properties like house/cottage/plant)
  ...
  // ✅ SAFE ALTERs (do NOT crash if column already exists)
  ...
  // ✅ REQUIRED: share DB + AWS clients so routes can use them
  app.set("db", db);
...
// =======================================================
// AUTH — SIGNUP / LOGIN / RESET  (kept as-is from your file)
// =======================================================
...
// =======================================================
// ALERT SYSTEM — UPLOAD + REKOGNITION (UPDATED: zone + cost SAFE)
// =======================================================
...
app.post("/api/trigger-alert", upload.single("image"), async (req, res) => {
  try {
    ...
    // Rekognition (may fail if env/iam missing -> handled)
    let matches = [];
    try {
    ...
    let cost = 0.001;
    if (zoneId) {
      const zone = await db.get("SELECT cost_per_scan FROM zones WHERE id=?", zoneId);
    ...
    // Email (SAFE: do not crash if SES env missing)
    if (!isKnown && email) {
      try {
    ...
    res.json({ ok: true, faces: matches, cost, zone_id: zoneId });
  } catch (err) {
