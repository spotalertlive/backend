...
async function initDb() {
  ...
  if (row.cnt === 0) {
    ...
    await db.run("INSERT INTO plans (name, price, cameras, scan_limit) VALUES (?,?,?,?)", "Elite", 149.99, 20, 30000);
  }
  ...
  try {
    ...
    await db.run(
      `INSERT INTO users (name,email,password_hash,plan,trial_end)
       VALUES (?,?,?,?,?)`,
      name, email.toLowerCase(), hash, "Free Trial", trialEnd.toISOString()
    );
...
app.post("/api/auth/request-reset", async (req, res) => {
  try {
    ...
    await db.run(
      ...
      token, expires.toISOString(), email.toLowerCase()
    );
